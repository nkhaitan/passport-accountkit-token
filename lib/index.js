...
var AccountKitTokenStrategy = function (_OAuth2Strategy) {
  ...
  function AccountKitTokenStrategy(_options, _verify) {
    ...
    options.authorizationURL = options.authorizationURL || 'https://www.accountkit.com/v2.4/dialog/oauth';
  ...
  _createClass(AccountKitTokenStrategy, [{
     ...
     *   - `username`         the user's AccountKit username
     *   - `displayName`      the user's full name
     *   - `name.familyName`  the user's last name
     *   - `name.givenName`   the user's first name
     *   - `name.middleName`  the user's middle name
     *   - `gender`           the user's gender: `male` or `female`
     *   - `profileUrl`       the URL of the profile for the user on AccountKit
    ...
    value: function userProfile(accessToken, done) {
      ...
      this._oauth2.get(profileURL, accessToken, function (error, body, res) {
        ...
        try {
          ...
          var imageUrl = _url2.default.parse('https://graph.accountkit.com/' + json.id + '/picture');
